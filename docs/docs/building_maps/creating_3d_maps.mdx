...
- **3D Paint** adds persistent fixed-isometric paint layers over the 3D scene for organic details such as moss, cracks, dirt, grass, puddles, and generated brick/tile patterns.
...
8. For a fixed isometric presentation, use **3D Paint** to add organic beauty details over the clean 3D structure.
