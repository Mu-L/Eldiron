...
- **Surface Noise** applies procedural noise to selected 3D faces. Its **NOISE** HUD material slot can be assigned or cleared from the Tiles dock or the Palette dock's Art Palette controls, and the noise is evaluated from object/world-space coordinates so adjoining noisy faces can stay continuous around corners.
- **Iso Paint** adds persistent fixed-isometric paint layers over the 3D scene for organic details such as moss, cracks, dirt, grass, puddles, and generated brick/tile patterns.
...
8. For a fixed isometric presentation, use **Iso Paint** to add organic beauty details over the clean 3D structure.
